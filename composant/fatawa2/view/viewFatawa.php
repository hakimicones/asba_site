...
class viewFatawa {
	...
	$this->ariane = '{fatawa}';
		...
		$data .=	
			  		'<form action="'.$this->lg.'/fatawa/list-'.$this->id.'-0-30.html" method="post" id="form-search">'
					.$this->getSearchInput()
					.'<ul class="blog" style="padding-top: 80px;" '.$dir.'>'.$li .'</ul></form>  ';
	...
	$html = '<div class="rech"><input type="text" style="background-position:'.$p.'"  onBlur="cherche(this)" class="search_anim1 form-control col-sm-6" name="search" placeholder="{recherche}..."> '.$this->getListCat().' </div>' ;
	return $html;
	}
	/********************************************/
	...
	function getListCat() {
	$se   = '<!-- ajout --><div id="slc"><select onchange="this.form.submit();" name="cat" id="cat" class=" form-control "  style="margin-right: 10px;" > ';	
