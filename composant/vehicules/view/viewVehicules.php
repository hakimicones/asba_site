...
class viewVehicules {
	...
	$li = 	'<section id="vehicules"   style="margin-top: 20px;margin-bottom: 20px;">'
			.' '
			.'<div class="container">
			<form action="'.$this->lg.'/vehicules/list-'.$this->id.'-0.html" method="post">'
			.$this->getSearchInput()
			.'<ul id="vehicules-list" class="vehicules">';
		$ind= true;
		foreach ($this->rows  as $row )   { 
		 ...
		 $ind  = !$ind;
		  ...
		  $photo =(!empty( $row['photo']))?$row['photo'] :  'images/vehicules/voiture1.png';
	 ...
	 $l2   =	($row['quantite'])?'<div class="col-sm-4 min-80" > <div id="progressbar"> 
		...
		$li .= '<a href="'.$this->lg.'/vehicules/simulation-'.$this->id.'-'.$row['id'].'.html" 
		...
		<script src="composant/vehicules/assets/script.js" ></script>
		<script>
  ...
  <div class="modal-dialog" role="document">
	...
	$URL = ' https://www.alsalamalgeria.com/ar/simulations/list-18-0.html';
	...
	function getSimulation1() {
   
   
   
 
            $url = 'composant/vehicules/view/tpl/form.php'; 
   ...
   function getConv() {
	   ...
	   $se   = '<!-- ajout --><select  class="form-control " id="tauxCharge"  onchange="info()" data-live-search="true" > ';
