...
/****************************************/
		...
		//	console.log(capaciteNette+" -"+tauxCharge+" -"+nbEcheance+" -"+montantBien);
            ...
            //  Control de Capacité
			//console.log( parseFloat($("#salaireCoEmp").val() )  );
			var salaireCoEmp = ( !isNaN(parseFloat($("#salaireCoEmp").val().split(".").join("")))   ) ?parseFloat( $("#salaireCoEmp").val().split(".").join("")).toFixed(2) : 0 ;
			
            if (!isNaN(parseFloat($("#salaireEmp").val().split(".").join("")))  ) {
             var salaireEmp = parseFloat($("#salaireEmp").val().split(".").join(""))
			$("#salaireCumule").val( parseFloat(salaireEmp  + salaireCoEmp ) ) ;
                
			//	alert($("#salaireCumule").val());
				 if (t) {
				// alert(t);
				var salCum = $("#salaireCumule").val()
				if ( salCum < 50001 ) {$("#tauxCapacite").val(30);      }
        ...
        //    console.log("capaciteNette  :  "+capaciteNette+" tauxCharge : "+tauxCharge+" nbEcheance :  "+nbEcheance+" montantBien :  "+montantBien);
...
function affRubSelectAn(nom,lib,maxi,lg,tag,def) {
	...
	if (lg.toLowerCase()!='ar') { pos = 'style="text-align:right;"';} else{pos = 'style="float: right;text-align: left;"'; } 
	
	var div  = '<div class="col-md-9 my-input"><select '+tag+' class="form-control hasValue " id="'+nom+'" name="'+nom+'">'
	var label = '<label '+pos+' for="'+lib+'"  class="col-md-3 control-label">'+lib+'</label>';
    var opt = '';
	...
	if (lg.toLowerCase()!='ar' || te ) { html = label +div ;} else{ html = div  +label }
	
	return '<div   class="row r1">'+html+'</div> ' ;
}
/***********************/
...
function affRubSelect(nom,lib,option,lg,tag,css) {
	...
	var div  = '<div class="col-md-9 my"><select '+tag+' class="form-control hasValue" id="'+nom+'" name="'+nom+'">'
	var label = '<label '+pos+' for="'+lib+'"  class="col-md-3 control-label">'+lib+'</label>';
    var opt = '';
