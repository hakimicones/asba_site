...
public function __construct($c,$obj,$lg)
	...
	$script = ($this->detect->isMobile()) ? ' <script src="modules/menu-warda/js/mobile.js" type="text/javascript"></script>' : '';
	$li=$script.'<ul id="menu" class="menu_'.$this->lg.'">';
	$i = 0 ;
	$result = $this->getMenu();
	 foreach($result as $el) 
			 ...
			 $id_tag    = isset( $param->id_tag ) ?  'id = "'. $param->id_tag.'"' :"";
			  ...
			  $li .= '<li id="nav'.$i.'"><img src="'.$img .'.png" alt=""><span '.$id_tag.'  class="'.$css.'"><a href="#"> '.  $tit .'</span></a>'. $el['html'] .'</li>';
