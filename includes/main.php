...
private function getSafeInput($key, $default = null)
{
    ...
    public function getPages()
    {
        ...
        if ($this->conect())
        {
            ...
            $this->tmp = $this
                ->cfg->theme;
            if (isset($this->tmp) && !empty($this->tmp))
            {
            }
            else
            {
            ...
            if ($this->id == 0 && isset($this->lg))
            {
                $this->id = $m->getIdVedette($this->lg);
            }
            ...
            if (!empty($page))
            {
                ...
                {
                    $this->ispage = '';
                    $this->style_p = 'style="display:none;"';
                    $this->canonical = '<link rel="canonical" href="' . $this->lg . '/accueil.html" />';
                }
                else
                {
                    $this->ispage = '_page';
                    $this->style_p = (!empty($this
                        ->param
                        ->bg)) ? 'style="background: url(' . $this
                        ->param->bg . ') no-repeat  ;background-size:cover"' : 'style="display:none;"';
                }
                ...
                if (!empty($this->ispage))
                {
                    ...
                    {
                        $par = '<li><a href="' . $parent['link'] . '">' . $parent['title'] . "</a></li>";
                    }
                    ...
                    $this->ariane = '<li><a href="index.php"><i class="fa fa-home"></i></a></li>' . $par . $ariane;
                ...
                {
                    $this->affTitle = 'style="display:none"';
                }
                    ...
                    ->param->volet2 != 'false')
                {
                    ...
                    $pophome = (!$this->session) ? '/**' . $this->session . '**/' : "
                        ...
                        ->volet2) . "
								...
								if(window.matchMedia('(max-width:640px)').matches) {
										...
										$('#pop-home ').css('background',' url(" . $this
                        ->param->style3 . ")').css('background-size','contain').css('background-color','#fff').css('background-position','center').css('position','fixed').css('top','0');
								...
								$('#extra ').css('display','none');
								$('#pop-home ').css('background','url(" . $this
                        ->param->style2 . ")').css('background-size','contain').css('background-color','#fff').css('position','fixed').css('top','0');; 
                    ...
                    $this->script1 .= (!empty($this->ispage)) ? "
								...
								\$(document).ready(function () {  setTimeout(  lanceModal , " . $this
                        ->param->temps . ");     });
								function lanceModal() {\$('#PopUPModal').modal('show');}  
								}
								\$('#PopUPModal .modal-dialog ').css('width','" . $this
                        ->param->style . "');
								\$('#PopUPModal  .modal-content').css('height','" . $this
                        ->param->taille . "');
								\$('#PopUPModal  .modal-content').css('background',' url(" . $this
                        ->param->style2 . ")').css('background-size','contain').css('background-color','#fff');
								 
								//" . $this
                        ->param->volet2 : $pophome;
    ...
    public function getDataHtml()
    {
        ...
        $ext[] = (!$this->pageIntrouvale) ? $this->addPos : '';
        ...
        if (strtolower($this->lg) == 'ar')
        {
            $meta = '<html dir="rtl" lang="ar"  xml:lang="ar">';
        }
        else
        {
        ...
        $tag[] = '{meta_desc}';
        $ext[] = (!$this->pageIntrouvale) ? $this->meta_desc : '';

        $tag[] = '{keyword}';
        $ext[] = (!$this->pageIntrouvale) ? $this->keyword : '';
  ...
  /**************/
    ...
    public function getContent($id)
    {
        ...
        if (isset($cont->bg))
        {
            $this->style_p = $this->style_p = 'style="background: url(' . $cont->bg . ') no-repeat  ;background-size:cover"';
        }
    ...
    public function getArianeFin()
    {
        return '<section id="ariane"><ol class="breadcrumb container" dir=""> ' . $this->ariane . '</ol></section>';
    }
    ...
    public function getArabe()
    {
        ...
        {
            $st = '<link href="' . 'themes/' . $this->tmp . '/fonts_ar/arabe.css" rel="stylesheet" type="text/css" /> ' . '<link href="' . 'themes/' . $this->tmp . '/css/style_ar.css" rel="stylesheet" type="text/css" /> ' . " ";
        }
